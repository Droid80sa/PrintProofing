...
def upload():
    ...
    if request.method == "POST":
        ...
        if not customer_id:
            ...
            return render_upload_form(
                selected_designer_id=selected_designer_value,
                selected_customer_id=customer_id,
                job_name_value=job_name,
                notes_value=notes,
                notify_customer_checked=notify_flag,
        ...
        if not file:
            ...
            return render_upload_form(
                selected_designer_id=selected_designer_value,
                selected_customer_id=str(customer_record.id),
                job_name_value=job_name,
                notes_value=notes,
                notify_customer_checked=notify_flag,
                notify_subject_value=notify_subject,
        ...
        if notify_flag:
            ...
            if portal_enabled and (not credential or not credential.is_active):
                ...
                if uploader_user:
                    issued_by_user_id = uploader_user.id
                try:
                    ...
                    flash("⚠️ Customer invite could not be generated. The notification was sent without an invite link.", "warning")
        ...
        # Render a success template instead of raw HTML for better branding consistency
...
def submit():
    ...
    if proof:
        ...
        designer_name = proof.designer.display_name if proof.designer else ""
        designer_email = proof.designer.email if proof.designer else None
        if proof.designer and proof.designer.reply_to_email:
            reply_to = proof.designer.reply_to_email
        if proof.designer and proof.designer.email:
    ...
    try:
        send_email_notification(
            ...
            body,
            to_address,
            user=proof.designer.user if proof and proof.designer else None,
